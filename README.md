...
* 채팅방 생성, 조회, 입장, 퇴장
    ...
    * 채팅방 입장, 퇴장은 채팅방 인원에 영향을 주지는 않고 채팅창을 보고 있는지 여부를 구분하는 용도이다
...
이렇게 리소스를 제한함으로써 어느 환경에서 테스트하더라도 결과를 일관되게 만들 수 있다고 판단하였다.  
...
- cpus: "0.5"
- 메모리: 512M
- 디스크: 호스트 OS가 Windows 11 환경이다보니 디스크 크기나 I/O 속도를 제한하는데 제약이 있었다.
- 네트워크 대역폭: tc를 사용하여 네트워크 속도를 제한하려 했으나 호스트 OS가 Windows 11이라 적용하지 못했다.


성능 테스트를 진행하기에 앞서 도커로 웹 서버를 구동한 뒤, 사용자 수를 점진적으로 늘려야 정확한 수치가 나오기 시작했다.  
아마도 웹 서버가 부하를 받으면서 자원을 더 적극적으로 할당한 것으로 보인다. (예: JVM 워밍업, 스레드 풀의 동적 확장 등)  
사용자 수는 10명 단위로 증가시켰으며, 메시지 생성 시간은 20초로 설정하였다. 성능테스트 결과는 아래와 같다.
...
* webflux : 550명 이하 동시 접속까지 메시지 유실없이 처리가 가능하였다. (위 이미지는 600명 시뮬레이션 실패)

### 메시지 처리량

![tpsPanel](./docs/tpsPanel.png)

최대 접속자 수 지표에서 나온 것처럼 MVC 대비 처리량이 4배 이상 높은 것으로 확인되었다.

### CPU

![cpuPanel](./docs/cpuPanel.png)

webflux가 mvc보다 최대 접속자 수 및 메시지 처리량이 훨씬 좋았지만 CPU 사용률은 거의 동일한 것을 확인하였다.

### 스레드 수

![threadCtnPanel](./docs/threadCtnPanel.png)

WebFlux는 적은 스레드로 높은 처리량을 낼 수 있어, CPU 자원을 보다 효율적으로 활용할 수 있었다.
위 스레드 수 지표를 보면 webflux가 동시 접속자 수가 늘어도 일정한 스레드 수를 유지하는 것을 확인할 수 있다.

### 메모리와 GC 시간
메모리는 mvc와 webflux의 동시 접속자 수 및 테스트 횟수가 차이가 있어 비교가 무의미했다.  
GC 시간은 두 프로젝트 다 MajorGC 기준 500ms 이내로 별 차이가 없었다.

### 최대 응답 속도

![maxResponsePanel](./docs/maxResponsePanel.png)

HTTP API 응답 속도는 지표상으로만 보면 mvc가 빠른 것처럼 보이나 동시 접속자 수에 차이가 있다는 것을 감안하면 오히려 webflux가 더 높은 성능을 보이는 것으로 해석할 수 있다.


## 결론

WebFlux는 적은 리소스로 더 많은 클라이언트를 안정적으로 처리할 수 있었고, 특히 스레드 수가 적음에도 TPS와 안정성 측면에서 뛰어난 성능을 보였다. Spring MVC는 구조가 단순하고 직관적인 반면, 리소스 확장성 면에서는 한계가 있음을 확인할 수 있었다.  
성능 차이를 직접 확인하며 서버 구조 선택이 어플리케이션의 확장성과 효율성에 얼마나 큰 영향을 미치는지 경험할 수 있었다.