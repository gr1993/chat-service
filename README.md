...
최대 접속자 수 지표에서 나온 것처럼 webflux가 4배 이상 처리량이 높은 것으로 확인되었다.
...
위 쓰레드 수 지표를 보면 webflux가 동시 접속자 수가 늘어도 일정한 쓰레드 수를 유지하는 것을 확인할 수 있다.
...
HTTP API 응답속도는 지표상으로만 보면 mvc가 빠른것처럼 보이나 동시 접속자 수가 차이나는 것을 생각해보면 오히려 webflux가 더 높은 성능을 보이는 것으로 해석할 수 있다.
...
성능 차이를 직접 확인하며 서버 구조 선택이 어플리케이션의 확장성과 효율성에 얼마나 큰 영향을 미치는지 경험할 수 있는 좋은 경험이였다.